...
class Player:
    def __init__(self, screen, fps):
        ...
        self.regularity = 5
        # var 1
        # self.every_frame = [[(frame + 1) * self.regularity for frame in range(self.fps // self.regularity)], 0]
        # var 2
        self.every_frame = [[(frame + 1) * (self.fps // self.regularity) for frame in range(self.regularity)], 0]
        self.position = [0, 0]  # [x, y]
        self.screen = screen
        self.player_motion_step = 48 if self.fps == 30 else 24
    ...
    def draw_player(self, frame_rate):
        self.screen.blit(self.calculate_texture(frame_rate), (960 - self.player_sprite_size[0] // 2, 540 - self.player_sprite_size[1] // 2))
